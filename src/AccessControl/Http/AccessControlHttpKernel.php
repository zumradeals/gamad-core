...
/**
 * A distinct HTTP entry point from AdministrativeHttpKernel,
 * PersonsAndAccountsHttpKernel and OrganizationsAndMembershipsHttpKernel:
 * this context never imports any of them (ADR-0013 boundary). It
 * authenticates with the same session-token mechanism as Persons and User
 * Accounts and Organizations and Memberships — the concrete
 * AuthenticationAdapter is wired in at the composition root
 * (public/index.php), never referenced by class name here. Every route in
 * this context requires a session, so `requiredScopes` is always
 * non-empty in AccessControlRoutes.
 */
final readonly class AccessControlHttpKernel
{
    public function __construct(
        private OpenApiRequestValidator $validator,
        private AccessControlResponseValidator $responseValidator,
    ...
    public function handle(Request $request): Response
    {
        ...
        try {
            ...
            $this->record($actor->actorId, 'unhandled_access_control_request', $request->path, 500, $request, ['exception' => $exception::class]);
