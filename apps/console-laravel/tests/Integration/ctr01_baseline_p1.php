...
/**
 * Vérifie que CTR-01 initialise un index vide depuis la baseline opérationnelle,
 * sans lecture du corpus Genesis II ni appel au parseur Ingestion.
 *
 * Exécution depuis la racine du dépôt :
 *   php apps/console-laravel/tests/Integration/ctr01_baseline_p1.php
 */
...
$temp = sys_get_temp_dir().'/gamad-ctr01-baseline-'.getmypid();
$fichiers = [
...
$secret = 'Secret-CTR01-Baseline-2026!';
...
echo "INTÉGRATION — CTR-01 PAR BASELINE P1\n\n";
...
$verifier(
    ! is_file($fichiers['index']),
    'l’index reste absent avant la première lecture CTR-01',
);

$identite = $requete('GET', '/api/v1/identites/AUT-GAMAD-001', null, $jeton);
$verifier(
    $identite['statut'] === 200
...
$seconde = $requete('GET', '/api/v1/identites/AUT-GAMAD-001', null, $jeton);
$entitesApres = (int) $pdo->query('SELECT count(*) FROM entite')->fetchColumn();
$verifier(
    $seconde['statut'] === 200 && $entitesApres === $entites,
    'une seconde lecture réutilise l’index sans reconstruction destructive',
);

$controleur = file_get_contents(
    $racine.'/apps/console-laravel/app/Http/Controllers/Ctr01Controller.php'
);
$verifier(
...
echo "\n";
if ($echecs === 0) {
    echo "CTR-01 par baseline P1 : ÉTABLI.\n";
    exit(0);
}

echo "CTR-01 par baseline P1 : NON ÉTABLI ({$echecs} écart(s)).\n";
exit(1);
