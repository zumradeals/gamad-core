...
/**
 * Connexion au registre persistant des contrats (CAP-CORE-009), physiquement
 * distinct de tout autre magasin.
 *
 * Variables dédiées :
 *   CONTRACT_REGISTRY_URL   PostgreSQL en exploitation ;
 *   CONTRACT_REGISTRY_PATH  SQLite en local et en CI.
 *
 * Aucune autre variable de magasin n'est consultée ici. Aucun repli
 * silencieux : en production, l'absence de `CONTRACT_REGISTRY_URL` doit
...
final class Magasin
{
    ...
    {
        $url = $chemin === null ? self::environnement('CONTRACT_REGISTRY_URL') : null;
        if (is_string($url) && $url !== '') {
            $p = parse_url($url);
            if ($p === false || !isset($p['host'])) {
                throw new \RuntimeException('CONTRACT_REGISTRY_URL invalide.');
            }
            $pdo = new \PDO(
                sprintf(
            ...
            $chemin ??= self::environnement('CONTRACT_REGISTRY_PATH')
    ...
    private static function environnement(string $nom): ?string
    {
        $configuration = match ($nom) {
            ...
            'CONTRACT_REGISTRY_PATH' => 'database.connections.gamad_contracts.database',
            default => null,
        };
        $valeurLaravel = null;
