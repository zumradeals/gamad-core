...
/**
 * Connexion au registre persistant des organisations (CAP-CORE-002),
 * physiquement distinct de tout autre magasin — en particulier du registre
 * d'identités (CAP-CORE-001) et du registre des mandats (CAP-CORE-003).
 *
 * Variables dédiées :
 *   ORGANIZATION_REGISTRY_URL   PostgreSQL en exploitation ;
 *   ORGANIZATION_REGISTRY_PATH  SQLite en local et en CI.
 *
 * Aucune autre variable de magasin n'est consultée ici. Aucun repli
 * silencieux : en production, l'absence de `ORGANIZATION_REGISTRY_URL` doit
 * échouer bruyamment (voir `EtatFondation::inspecterCible`), jamais retomber
 * sur SQLite sans que quiconque ne le décide explicitement.
 */
final class Magasin
{
    public static function connecter(?string $chemin = null): \PDO
    {
        ...
        SchemaOrganisations::migrer($pdo);
    ...
    {
        $url = $chemin === null ? self::environnement('ORGANIZATION_REGISTRY_URL') : null;
        if (is_string($url) && $url !== '') {
            $p = parse_url($url);
            if ($p === false || !isset($p['host'])) {
                throw new \RuntimeException('ORGANIZATION_REGISTRY_URL invalide.');
            }
            $pdo = new \PDO(
                sprintf(
            ...
            $chemin ??= self::environnement('ORGANIZATION_REGISTRY_PATH')
    ...
    private static function environnement(string $nom): ?string
    {
        $configuration = match ($nom) {
            ...
            'ORGANIZATION_REGISTRY_PATH' => 'database.connections.gamad_organizations.database',
            default => null,
        };
        $valeurLaravel = null;
